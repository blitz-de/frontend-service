    ...
    return variableUserState.users?.map((user, index) => {
        return (
            <div key={index} className="col-lg-3 mb-3 d-flex align-items-stretch">
                <div className="card">
                    <Link to={"/detail/"+user.username}><img src={baseUrl+user.profile_photo} className="card-img-top" alt="..." /></Link>
                    <div  className="card-body">
                        <h5> Name:
                            
                            <Link className="link-secondary text-decoration-none" to={"/detail/"+user.username}>{" " +user.full_name}</Link>
                        </h5>
